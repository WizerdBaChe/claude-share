...
# the same reason). The task name below is a placeholder for the source's own
# daily graph-watchdog task, which names nothing an adopter has; this whole
# check has nothing to run against in this share.
schtasks /Query /TN "<graph-watchdog-task>" /FO LIST | findstr "Status Next"   # task exists, next run scheduled
