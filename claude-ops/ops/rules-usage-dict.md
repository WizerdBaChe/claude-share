...
| 規則該放哪一層 | 本檔第一節判別法 |
| 寫 hook／skill／`rules/*.md`／專案 CLAUDE.md／路由條目時，某條全域原則具體要求這個小物件長什麼樣 | `ops/references/principle-design-guide.md`（依物件類別查 AP 索引；每條帶 `detect:`） |
| 新增一筆分類／路由條目：該填哪些欄位、這個面表達不了的欄位放哪 | `ops/references/entry-schema.md` §3–§5（14 面適配表；lint 為 source-only 工具，未隨此 repo 收錄） |
...
| manual-acceptance checklist（人工驗收清單／UAT） | 兩段：`A. 必驗` + `B. 體驗`，跨段連續編號。**欄位形狀、上限值、兩條 rank 軸、收錄閘門一律以 `ops/references/uat.md` §1 P1–P9 為唯一出處，本表不複述**（複述過的版本會在上限變動那天無聲地與它分歧；2026-09-09 收斂） | `ops/references/uat.md` | 交付含「只有人跑得出／看得出來」的部分時（全域 CLAUDE.md `[BC]`；L1/L2 有 boundary contract 時併入其 §3，不另開一份） |
...
| self-declaration（自我宣告塊） | 成熟度（兩把尺各帶來源）/ 保真度（逐儀器）/ **算過的·只畫過的·沒做的** / 最弱的一環（一句）/ 來源 sha / 界線→refutability；塊內不得以 `<n> PASS` 當可信度訊號 | `60-record-templates.md` §4 | 人會據以行動、且不會自己重推的**耐久產物**（報告、簡報、受眾頁、設計文件、案卷、評估）；聊天回答、自帶 meta 的機器紀錄、程式變更**不欠** |
...
| routing/definition entry（分類／路由條目） | 核心 `id / owner / status / kind / detect`；`kind: routing` 再加 `trigger / on-fire`；各面以自身既有拼法承載，表達不了的欄位放適配表指定的 carrier（如 SKILL.md 的 class/source → trigger-class 區塊、hook 的 status → docstring `STATUS:` 行）；缺值寫字面 `none`，不留空；2026-09-08 後出生的條目缺核心欄位＝FAIL，舊條目 WARN 且數量只降不升 | `ops/references/entry-schema.md` §3–§4（lint 為 source-only 工具，未隨此 repo 收錄；integrity-sweep check 29；2026-09-08 登記） | 在 14 個分類／路由面任一新增條目時（hook、`rules/*.md`、trigger-class 區塊、registry 列、dict 節、OPS／inbound 列、page class） |
