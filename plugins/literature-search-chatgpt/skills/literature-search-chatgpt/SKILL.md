...
description: >-
  ...
  scientific-research-guide). Target runtime: the ChatGPT/Codex plugin; invoke explicitly
  when this skill is requested. A future Claude adapter is maintained separately.
---
...
Eight of these ten reference files ship with this copy of the skill; load each on
...
`references/connectors.md` and its connector registry/tool lane — machine-bound to this owner's
...
- `references/connectors.md` — **not shipped in this repo.** P2 routing for registered
  ...
  `LSE_HOST_POLICY`) is a separate optional policy input; this package does not ship a
  machine-wide hook or a default host-policy file.
